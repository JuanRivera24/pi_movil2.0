document.addEventListener("DOMContentLoaded", function () {
  ...
  const barberoContainer = document.getElementById(
    "barbero-selector-container"
  );
  ...
  function filtrarBarberosPorSede(idSede) {
    ...
    const barberosFiltrados = todosLosBarberos.filter(
      (b) => b.sede.idSede == idSede
    );
  ...
  async function cargarCitasPorBarbero(barberoId) {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/citas/barbero/${barberoId}`
      );
      ...
      citasTableBody.innerHTML =
        '<tr><td colspan="6" class="text-center">Error al cargar las citas.</td></tr>';
    }
  }
